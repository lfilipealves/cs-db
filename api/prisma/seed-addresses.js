    ...
    for (const addressEntry of addressesData) {
        ...
        for (const address of addressEntry.addresses) {
            ...
            if (!endereco) {
                ...
                console.log(`Endereço ${address.title} criado com sucesso: ID${newAddress.id}`)
            } catch (error) {
            ...
            console.log(`Endereços inseridos para ${user.name}:`,
            userWithAddresses.addresses.length > 0
            ? userWithAddresses.addresses.map(a=> `${a.title} (${a.id})`).join(",")
